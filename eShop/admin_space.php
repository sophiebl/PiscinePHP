...
if ($_SESSION['status'] == "admin")
{
	...
	if (isset($_POST['submit_add_stock']) && $_POST['submit_add_stock'] == "Ajouter")
	{
		...
		{
			$query = "INSERT INTO item (item_name, type, description, item_stock, price) VALUES ('".mysqli_real_escape_string($db, $_POST['item_name'])."', '".mysqli_real_escape_string($db, $_POST['type'])."', '".mysqli_real_escape_string($db, $_POST['description'])."', '".mysqli_real_escape_string($db, $_POST['item_stock'])."', '".mysqli_real_escape_string($db, $_POST['price'])."')";
...
<form action="admin_space.php" method="POST">
	<tr>
		...
		<td><input type="text" name="type" placeholder="Categorie du produit" value=""></td>
...
<table>
	<th>
		...
		<td class="bold">Prix(au kg)</td>
		<td class="bold">Etat commande</td>
	</th>
	<?PHP
